...
const checkout = require('../../models/bill/checkoutVNPAY.model')
...
router.post('/create_payment_url', function (req, res, next) {
    ...
    res.status(200).json({ url: vnpUrl, data: vnp_Params })
...
router.get('/vnpay_return', function (req, res, next) {
    ...
    if (secureHash === signed) {
        ...
        res.status(200).json({ RspCode: 'success', code: vnp_Params['vnp_ResponseCode'] })
    } else {
        res.status(200).json({ RspCode: 'success', code: '97' })
...
router.get('/vnpay_ipn', function (req, res, next) {
    ...
    let checkAmount = Amount; // Kiểm tra số tiền "giá trị của vnp_Amout/100" trùng khớp với số tiền của đơn hàng trong CSDL của bạn
            ...
            if (checkAmount) {
                if (paymentStatus == "0") { //kiểm tra tình trạng giao dịch trước khi cập nhật tình trạng thanh toán
                    if (rspCode == "00") {
                        ...
                        res.render('checkout')
                        // res.status(200).json({ data: vnp_Params, checkOrderId, checkAmount })
                    }
                    else {
                        //that bai
                        //paymentStatus = '2'
